...
function App() {
  ...
  const fetchPersonas = async () => {
    try {
      const response = await Persona.getAllPersonas();
  ...
  const handleDeletePersona = async (id) => {
    if (window.confirm("¿Está seguro de eliminar esta persona?")) {
      try {
        await Persona.deletePersona(id);
        fetchPersonas();
      } catch (error) {
  ...
  const handleFormSubmit = async (persona) => {
    try {
      if (persona.Id) {
        await Persona.updatePersona(persona);
      } else {
        await Persona.createPersona(persona);
      }
      fetchPersonas();
      setShowForm(false);
    } catch (error) {
    ...
    <div className="container mt-4">
      ...
      <Modal show={showForm} onHide={() => setShowForm(false)}>
        <Modal.Header closeButton>
          <Modal.Title>
            {selectedPersona ? "Editar Persona" : "Agregar Persona"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Create
